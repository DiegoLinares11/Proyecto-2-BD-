...
const crearUsuariosBulk = async (req, res) => {
  try {
    ...
    const result = await User.insertMany(usuarios);
    res.status(201).json(result);
  } catch (error) {
    res.status(500).json({ error: 'Error creando los usuarios', message: error.message });
  }
};

// Obtener todos los usuarios con filtros, ordenamiento y paginación
const obtenerUsuarios = async (req, res) => {
  const { page = 1, limit = 10, nombre, edad, genero, sortBy = 'fechaRegistro' } = req.query;
  try {
    const filtros = {};
    if (nombre) filtros.nombre = { $regex: nombre, $options: 'i' };
    if (edad) filtros.edad = edad;
